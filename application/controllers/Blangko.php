...
class Blangko extends CI_Controller{
	...
	function index(){
		...
		$this->load->view('blangko/blangko', $data);
	...
	function tambah(){
        ...
        if ( ! $this->upload->do_upload('file')){
			...
			$insert=array(
         	...
         	$this->Dokumen_m->insert_data("template_dokumen",$insert);
	...
	function get_data_blangko(){
		...
		$whereconditon='id='.$id;
		$data=$this->Dokumen_m->get_data_where($whereconditon, "template_dokumen");
	...
	function edit(){
        ...
        $this->load->helper('url', 'form'); 
        unlink("template/".$data[0]->kode.".rtf");
        $this->load->library('upload', $config);
        if ( ! $this->upload->do_upload('file')){
                $error = array('error' => $this->upload->display_errors());
                var_dump($error);
			...
			//$edit=array('kode' =>$nama_file);
         	//$this->Dokumen_m->update_data($whereconditon, "template_dokumen", $edit);
         	//echo json_encode($insert);
        }
	}
	function hapus_blangko(){
		...
		$whereconditon='id='.$id;
		$data=$this->Dokumen_m->get_data_where($whereconditon, "template_dokumen");
		$hapus=$this->Dokumen_m->delete_data($whereconditon, "template_dokumen");
        unlink("template/".$data[0]->kode.".rtf");
	}
	function ubah(){
		$id=base64_decode($this->input->post('id'));
