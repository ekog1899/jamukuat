<div class="w3-row"><h3>Managemen Blangko</h3></div>
      ...
      <div class="w3-row">
        <div class="w3-container" id="div_modal_detail">
        	<div class="w3-row" id="inputan_tambah">
        		<form id="myDropzoneDokumen" action="<?php echo base_url()?>blangko_eksekusi/tambah" class="dropzone">
      ...
      <div class="w3-row">
        <div class="w3-container" id="div_modal_detail_edit">
        	<div class="w3-row" id="inputan_edit">
        		<form id="myDropzoneDokumen_edit" action="<?php echo base_url()?>blangko_eksekusi/edit" class="dropzone">
...
function edit_blangko(id){
	document.getElementById("modal_detail").style.display="none";
        ...
        xhr.open("POST","<?php echo base_url('blangko_eksekusi/get_data_blangko')?>", true);
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function () {
            if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200){
            	var obj = JSON.parse(xhr.responseText);
							document.getElementById("url_template").href = "template_eksekusi/"+obj[0].kode+".rtf";
...
function hapus_blangko(){
  ...
  if (conf == true) {
    var xhr = new XMLHttpRequest();
    xhr.open("POST","<?php echo base_url('blangko_eksekusi/hapus_blangko')?>", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
      if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200){
      	document.getElementById("modal_detail_edit").style.display="none";
...
function tampilan(){
    var xhr = new XMLHttpRequest();
    xhr.open("POST","<?php echo base_url('blangko_eksekusi/list_kategori')?>", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
      if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200){
...
function ubah_nama(isi){
		var id=document.getElementById("template_id").value;
    var xhr = new XMLHttpRequest();
    xhr.open("POST","<?php echo base_url('blangko_eksekusi/ubah')?>", true);
    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
      if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200){
      	tampilan();
    ...
    xhr.send("id=" + btoa(id)+"&isi=" + btoa(isi)+"&tabel=" + btoa("template_dokumen_eksekusi")+"&field=" + btoa("nama"));
}
function ubah_jenis(isi){
    ...
    xhr.send("id=" + btoa(id)+"&isi=" + btoa(isi)+"&tabel=" + btoa("template_dokumen_eksekusi")+"&field=" + btoa("jenis_blangko_id"));
}
</script>
