...
<div class="container-fluid"> 
        <div class="card-header py-3">
            <h6 class="m-0 font-weight-bold text-primary">DATA PERMOHONAN KE KPKNL (PA)</h6>
        </div>
        <div class="card-body">
            <div class="row">
                <div class="col mr-2">
    ...
    <div class="modal" id="ModalUploadEdoc">
        <div class="modal-dialog modal-xl">
            ...
            <div class="modal-body" id="isi_modal_uploadEdoc">
              ...
              <hr>
              <form id="myDropzoneDokumenEdoc" action="<?php echo base_url('kpknl/upload_edoc')?>" class="dropzone">
...
<script type="text/javascript">
    function kpknl_tambah(){
        $("#ModalLoading").modal("show");
        var b = new XMLHttpRequest();
        b.open("POST", "<?php echo base_url()?>kpknl/kpknl_add", true);
        ...
        b.onreadystatechange = function () {
            if (b.readyState == XMLHttpRequest.DONE && b.status == 200){
                ...
                new SlimSelect({select: '#pilihan_jenis_kpknl'});
    ...
    function kpknl_detail(id){
        $("#ModalLoading").modal("show");
        var b = new XMLHttpRequest();
        b.open("POST", "<?php echo base_url()?>kpknl/kpknl_detail", true);
        b.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        b.onreadystatechange = function () {
            if (b.readyState == XMLHttpRequest.DONE && b.status == 200){
                var c = b.responseText;
    ...
    function pilih_jenis_kpknl(isi){
        let myArray = isi.split("^");

        document.getElementById("jenis_kpknl_id").value=myArray[0];
        document.getElementById("jenis_kpknl").value=myArray[1];
    ...
    function kpknl_save(){
        ...
        var data=serialize(f_kpknl);
        var url='<?php echo base_url("kpknl/kpknl_save")?>';
    ...
    function kpknl_delete(id){
        var conf = confirm("Apakah anda yakin akan menghapus data ini?");
        if(conf == true){
            $("#ModalLoading").modal("show");
            var b = new XMLHttpRequest();
            b.open("POST", "<?php echo base_url("kpknl/kpknl_delete")?>", true);
            b.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            b.onreadystatechange = function (){
                if (b.readyState == XMLHttpRequest.DONE && b.status == 200){
                    location.reload();
    ...
    function kirim(id){
        var xhr = new XMLHttpRequest();
        xhr.open("POST","<?php echo base_url("kpknl/kpknl_kirim")?>", true); 
        xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
        xhr.onreadystatechange = function() {
            if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
                var pesan=xhr.responseText;
