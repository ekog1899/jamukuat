        ...
        <form>
            ...
            <table class="table table-bordered table-sm table-hover" >
                <?php
                    ...
                    echo "<tr><td class='bg-primary text-light'>Keterangan</td><td><textarea class='form-control' onchange=edit_isi('keterangan',".$m->id.",this.value)>".$m->keterangan."</textarea></td></tr>";
                    echo "<tr><td class='bg-primary text-light'>Instansi</td><td>".$m->nama_mitra."</td></tr>";
                    echo "<tr><td class='bg-primary text-light'>Email</td><td><input class='form-control' value='".$m->email_satker."' onchange=edit_isi('email_satker',".$m->id.",this.value)></td></tr>";
                    echo "<tr><td class='bg-primary text-light'>Nomor WA</td><td><input class='form-control' value='".$m->wa_satker."' onchange=edit_isi('wa_satker',".$m->id.",this.value)></td></tr>";
                    echo "<tr><td class='bg-primary text-light'>E-doc Permohonan</td><td>";
                    if(strlen($m->edoc)>=4){
                        $isi_div_edoc='<p><b>E-Doc Permohonan</b></p><embed src="'.base_url($m->edoc).'" width="100%" height="300px">';
                    }else{
                        $isi_div_edoc='';
                    }
                    echo "<span class='btn text-primary  border border-primary btn-sm' onclick='tambah_edoc(".$m->id.")'><i class='fa fa-upload'></i> Upload Edoc Permohonan</span>";
                    echo "</td></tr>";
                    echo "<tr><td colspan=2>";
                        echo "<div id='pesan'><center></center></div>";
                        echo "<div><center>";
                        if($m->status==0){
                            echo "<span class='btn btn-danger btn-sm' onclick='pengamanan_delete(".$m->id.")'>Hapus</span>";
                        }

                        echo "<span class='btn btn-success btn-sm ml-2' onclick='kirim(".$m->id.")'>Kirim Pemberitahuan</span>";
                        echo "</center></div>";
                    echo "</td></tr>";
