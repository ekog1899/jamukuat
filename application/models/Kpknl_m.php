...
class Kpknl_m extends CI_Model{
    function __construct(){
        parent::__construct();
    }
    function get_kpknl_all(){
		$this->db->select('*');
    ...
    function get_jenis_kpknl(){
		$this->db->select('*');
		$this->db->from('master_jenis_kpknl');
		$this->db->order_by('jenis_kpknl_id', 'ASC');
		return $this->db->get()->result();
	}
    function get_instansi_kpknl($pa_id){
		$this->db->select('*');
    ...
    function get_kpknl_satker($user_id){
		$this->db->select('*');
		$this->db->select('convert_tanggal_indonesia(tanggal) AS tanggal_indo');
		$this->db->from('data_permohonan');
    ...
    function get_kpknl_kpknl($mitra_id){
		$this->db->select('*');
		$this->db->select('convert_tanggal_indonesia(tanggal) AS tanggal_indo');
		$this->db->from('data_permohonan');
